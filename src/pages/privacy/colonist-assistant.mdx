...
- possible card states made from those events;
- a short game summary; and
- a bounded set of current-game decision diagnostics, including board
  snapshots, search candidates, and timing data, used to replay and diagnose
  advice or automation failures.

It keeps current-game events, card states, the game summary, and decision
diagnostics in your local Chrome profile. Starting a different game replaces
the prior game's local records. It may keep settings in Chrome Sync.
...
Use **Reset session** to clear the current game's events, card states, summary,
and decision diagnostics. Remove the extension to delete all of its local
data. You can clear synced settings through Chrome.
